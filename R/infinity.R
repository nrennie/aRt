...
#' ggplot object in the shape of an infinity symbol.
#'
#' @param n Number of lines per colour. Default 25.
#' @param col_scheme Colour scheme of art. One of c("mono", "rainbow). Default
#' 0.1.
...
infinity <- function(n = 25,
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_path(
      data = df1_random,
      mapping = ggplot2::aes(x = .data$x, y = .data$y, colour = I(.data$col_choice))
    ) +
    ggplot2::xlim(-1, 1) +
    ggplot2::ylim(-1, 1) +
    ggplot2::coord_fixed() +
