...
generate_line <- function(x, y, length, angle, group = 1) {
  ...
  if (angle == "h") {
    output <- data.frame(x = x - 0.5 * length,
                         xend = x + 0.5 * length,
                         y = y,
                         yend = y,
                         group = group)
...
lines <- function(n = 100,
  ...
  plot_data <- data.frame(x = c(),
                          ...
                          group = c())
  for (i in 1:n) {
    k <- generate_line(x = stats::runif(1, 0, 30),
  ...
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = .data$x,
    ...
    ggplot2::coord_fixed() +
    ggplot2::theme_void() +
    ggplot2::xlim((min(c(plot_data$x, plot_data$y, plot_data$xend, plot_data$yend))),
                  (max(c(plot_data$x, plot_data$y, plot_data$xend, plot_data$yend)))) +
    ggplot2::ylim((min(c(plot_data$x, plot_data$y, plot_data$xend, plot_data$yend))),
                  (max(c(plot_data$x, plot_data$y, plot_data$xend, plot_data$yend)))) +
