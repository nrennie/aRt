...
moire <- function(inner_n = 20,
                  ...
                  outer_r = 0.2) {
  # inner square
  main1 <- expand.grid(x = seq(1, inner_n, by = 2),
  ...
  # plot
  p <- ggplot2::ggplot() +
    ggforce::geom_circle(data = main_square,
                         mapping = ggplot2::aes(x0 = .data$x, y0 = .data$y, r = inner_r),
                         fill = inner_col, colour = NA) +
    ggforce::geom_circle(data = topright,
                         mapping = ggplot2::aes(x0 = .data$x, y0 = .data$y, r = outer_r),
