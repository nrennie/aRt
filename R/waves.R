...
#' @param main_col Colour of lines. Either a single colour or
#' one of the colour palettes from rcartocolor. Default "black".
...
waves <- function(a = 23, b = 6, main_col = "black", bg_col = "white", s = 2021) {
  ...
  if (main_col %in% all_palettes) {
    p <- ggplot2::ggplot(df, ggplot2::aes(x = .data$x, y = .data$y)) +
      ...
      rcartocolor::scale_color_carto_c(type = "diverging",
                                       palette = main_col, direction = -1) +
      ggplot2::coord_polar() +
    ...
    p <- ggplot2::ggplot(df, ggplot2::aes(x = .data$x, y = .data$y)) +
      ggplot2::geom_path(colour = main_col) +
      ggplot2::coord_polar() +
      ggplot2::theme(panel.background = ggplot2::element_rect(fill = bg_col, colour = bg_col),
            plot.background = ggplot2::element_rect(fill = bg_col, colour = bg_col),
            plot.title = ggplot2::element_blank(),
